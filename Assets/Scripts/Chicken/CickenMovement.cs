...
namespace HurricaneVR.Framework.Core
{
    ...
    public class CickenMovement : MonoBehaviour
    {
        ...
        public HVRGrabbable HVR_Parent;
        ...
        [Tooltip("Distanz die das Huhn von Boden braucht um wieder loszulaufen.")]
        public float groundDistance = 0.4f;

        [Tooltip("Die Layer des Bodens")]
        public LayerMask ground_LM;

        [Tooltip("Die Layer, nach der das Huhn sucht.")]
        ...
        private void Awake()
        {
            ...
            if (!HVR_Parent) HVR_Parent = GetComponentInParent<HVRGrabbable>();

            #endregion HVR Grabber
        }
        ...
        public void Update()
        {
            ...
            if (timer >= wanderTimer && isAtFurniture == false && isGrounded) //Abklingzeit vom Laufpunkten
            {
            ...
            #region Stop Moving When Grabbed 

            if(HVR_Parent.IsBeingHeld == true && isBeingHold == false)
            {
                isBeingHold = true;
                NavMeshManager(isBeingHold);
                Debug.Log("Yea grabbed!");
            }
            else if(HVR_Parent.IsBeingHeld == false && isBeingHold == true && isGrounded)
            {
                isBeingHold = false;
                NavMeshManager(isBeingHold);
                Debug.Log("Continue pls");
            }

            #endregion Stop Moving When Grabbed

            #region Ground Check 

            isGrounded = Physics.CheckSphere(groundcheck.position, groundDistance, ground_LM);

            #endregion GroundCheck

        }

        private void NavMeshManager(bool hold)
        {
            if (hold == true)
            {
                //Disable Nav Mesh 
